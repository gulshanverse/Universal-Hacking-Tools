...
subcategory: Web server assessment
difficulty: Beginner
license: GPL-2.0
...
> Web server misconfiguration checks. This page is written for lawful, authorized security education.

## Overview

**Nikto** is used for identifying known exposures and unsafe defaults in a controlled assessment. Its value depends on clear authorization, a defined scope, and a documented assessment objective. It should be treated as one component of a broader workflow rather than as proof that a finding is exploitable or material.
...
| Category | Web Security |
| Subcategory | Web server assessment |
| Primary purpose | Web server misconfiguration checks |
| License | GPL-2.0 |
| Open source | Verify upstream; edition and component terms may differ. |
| Platform | Linux; Windows; macOS |
| Language | Perl |
| Repository | [Nikto repository](https://github.com/sullo/nikto) |
| Official website | [https://cirt.net/Nikto2](https://cirt.net/Nikto2) |
| Documentation | [https://github.com/sullo/nikto/wiki](https://github.com/sullo/nikto/wiki) |
| Current version | Not recorded here; verify upstream. |
| Difficulty | Beginner |
| Security domain | Web Security; Web server assessment |
| Defensive / offensive / both | Both; use only within authorization. |

## Purpose

Use Nikto when the assessment question matches **web server misconfiguration checks**. A professional workflow normally records the asset or artifact, authorization, time window, operator, configuration, result, confidence, and remediation owner.

## Key Features

* Identifying known exposures and unsafe defaults in a controlled assessment.
...
```mermaid
flowchart LR
    A[Authorized scope or artifact] --> B[Nikto]
...
* Focused workflow for web server misconfiguration checks.
...
Nikto cannot replace authorization, asset ownership, threat modeling, manual validation, secure coding review, incident-response judgment, or remediation verification. It does not establish attribution or business impact by itself.
...
| --- | --- | --- | --- |
| Nuclei | A related workflow or comparison point | Verify upstream | Verify upstream |
| OWASP ZAP | Complementary analysis | Verify upstream | Verify upstream |
...
Use Nikto when the objective is clearly defined, the data or target is authorized, the expected output is understood, and a reviewer can reproduce the observation.
...
1. [Nikto official repository](https://github.com/sullo/nikto)
2. [Nikto official website](https://cirt.net/Nikto2)
3. [Nikto official documentation](https://github.com/sullo/nikto/wiki)
