...
> Automated OSINT collection. This page is written for lawful, authorized security education.

## Overview

**SpiderFoot** is used for correlating public indicators, modules, and findings into an investigation graph. Its value depends on clear authorization, a defined scope, and a documented assessment objective. It should be treated as one component of a broader workflow rather than as proof that a finding is exploitable or material.
...
| Category | OSINT |
| Subcategory | OSINT automation |
| Primary purpose | Automated OSINT collection |
| License | MIT |
| Open source | Verify upstream; edition and component terms may differ. |
| Platform | Linux; Windows; macOS |
| Language | Python |
| Repository | [SpiderFoot repository](https://github.com/smicallef/spiderfoot) |
...
| Difficulty | Intermediate |
| Security domain | OSINT; OSINT automation |
| Defensive / offensive / both | Both; use only within authorization. |

## Purpose

Use SpiderFoot when the assessment question matches **automated osint collection**. A professional workflow normally records the asset or artifact, authorization, time window, operator, configuration, result, confidence, and remediation owner.

## Key Features

* Correlating public indicators, modules, and findings into an investigation graph.
...
```mermaid
flowchart LR
    A[Authorized scope or artifact] --> B[SpiderFoot]
...
* Focused workflow for automated osint collection.
...
SpiderFoot cannot replace authorization, asset ownership, threat modeling, manual validation, secure coding review, incident-response judgment, or remediation verification. It does not establish attribution or business impact by itself.
...
| --- | --- | --- | --- |
| theHarvester | A related workflow or comparison point | Verify upstream | Verify upstream |
| Amass | Complementary analysis | Verify upstream | Verify upstream |
| Recon-ng | Validation or defense | Verify upstream | Verify upstream |
...
Use SpiderFoot when the objective is clearly defined, the data or target is authorized, the expected output is understood, and a reviewer can reproduce the observation.
...
1. [SpiderFoot official repository](https://github.com/smicallef/spiderfoot)
2. [SpiderFoot official website](https://www.spiderfoot.net/)
3. [SpiderFoot official documentation](https://www.spiderfoot.net/documentation/)
