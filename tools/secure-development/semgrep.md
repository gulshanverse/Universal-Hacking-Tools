...
difficulty: Beginner
license: LGPL-2.1; components vary
...
> Code pattern analysis. This page is written for lawful, authorized security education.

## Overview

**Semgrep** is used for finding insecure code patterns and enforcing secure-development rules in local and ci workflows. Its value depends on clear authorization, a defined scope, and a documented assessment objective. It should be treated as one component of a broader workflow rather than as proof that a finding is exploitable or material.
...
| Category | Secure Development |
| Subcategory | Static analysis |
| Primary purpose | Code pattern analysis |
| License | LGPL-2.1; components vary |
| Open source | Verify upstream; edition and component terms may differ. |
| Platform | Linux; macOS; Windows |
| Language | OCaml; Python |
| Repository | [Semgrep repository](https://github.com/semgrep/semgrep) |
| Official website | [https://semgrep.dev/](https://semgrep.dev/) |
| Documentation | [https://semgrep.dev/docs/](https://semgrep.dev/docs/) |
| Current version | Not recorded here; verify upstream. |
| Difficulty | Beginner |
| Security domain | Secure Development; Static analysis |
| Defensive / offensive / both | Both; use only within authorization. |

## Purpose

Use Semgrep when the assessment question matches **code pattern analysis**. A professional workflow normally records the asset or artifact, authorization, time window, operator, configuration, result, confidence, and remediation owner.

## Key Features

* Finding insecure code patterns and enforcing secure-development rules in local and CI workflows.
...
```mermaid
flowchart LR
    A[Authorized scope or artifact] --> B[Semgrep]
...
* Focused workflow for code pattern analysis.
...
Semgrep cannot replace authorization, asset ownership, threat modeling, manual validation, secure coding review, incident-response judgment, or remediation verification. It does not establish attribution or business impact by itself.
...
| --- | --- | --- | --- |
| CodeQL | A related workflow or comparison point | Verify upstream | Verify upstream |
| Bandit | Complementary analysis | Verify upstream | Verify upstream |
| SonarQube | Validation or defense | Verify upstream | Verify upstream |
...
Use Semgrep when the objective is clearly defined, the data or target is authorized, the expected output is understood, and a reviewer can reproduce the observation.
...
1. [Semgrep official repository](https://github.com/semgrep/semgrep)
2. [Semgrep official website](https://semgrep.dev/)
