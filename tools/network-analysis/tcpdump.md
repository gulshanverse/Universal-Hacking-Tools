...
official_website: https://www.tcpdump.org/
documentation: https://www.tcpdump.org/manpages/tcpdump.1.html
security_domains: [Network Security, Packet capture]
dual_use: true
...
sources:
  official_repository: https://github.com/the-tcpdump-group/tcpdump
  official_website: https://www.tcpdump.org/
  official_documentation: https://www.tcpdump.org/manpages/tcpdump.1.html
status: Verify against upstream documentation.
---

# tcpdump

> Command-line packet capture. This page is written for lawful, authorized security education.

## Overview

**tcpdump** is used for focused capture and filtering for troubleshooting, incident response, and lab exercises. Its value depends on clear authorization, a defined scope, and a documented assessment objective. It should be treated as one component of a broader workflow rather than as proof that a finding is exploitable or material.
...
| Category | Network Security |
| Subcategory | Packet capture |
| Primary purpose | Command-line packet capture |
| License | BSD-3-Clause |
| Open source | Verify upstream; edition and component terms may differ. |
| Platform | Linux; macOS; Windows |
| Language | C |
| Repository | [tcpdump repository](https://github.com/the-tcpdump-group/tcpdump) |
| Official website | [https://www.tcpdump.org/](https://www.tcpdump.org/) |
| Documentation | [https://www.tcpdump.org/manpages/tcpdump.1.html](https://www.tcpdump.org/manpages/tcpdump.1.html) |
| Current version | Not recorded here; verify upstream. |
| Difficulty | Intermediate |
| Security domain | Network Security; Packet capture |
| Defensive / offensive / both | Both; use only within authorization. |

## Purpose

Use tcpdump when the assessment question matches **command-line packet capture**. A professional workflow normally records the asset or artifact, authorization, time window, operator, configuration, result, confidence, and remediation owner.

## Key Features

* Focused capture and filtering for troubleshooting, incident response, and lab exercises.
...
```mermaid
flowchart LR
    A[Authorized scope or artifact] --> B[tcpdump]
...
* Focused workflow for command-line packet capture.
...
tcpdump cannot replace authorization, asset ownership, threat modeling, manual validation, secure coding review, incident-response judgment, or remediation verification. It does not establish attribution or business impact by itself.
...
For network security, defenders should understand the observable activity without attempting to evade it. Correlate flow, DNS, IDS, authentication, and endpoint telemetry with packet timestamps. Segment networks, restrict unnecessary services, encrypt traffic, patch infrastructure, and tune detections.

## Detection

Correlate flow, DNS, IDS, authentication, and endpoint telemetry with packet timestamps. Useful telemetry may include application or service logs, DNS and network records, endpoint process events, identity events, cloud audit logs, or file-integrity data, depending on the tool and the environment. Establish a known-good baseline before interpreting anomalies.

## Mitigation

Segment networks, restrict unnecessary services, encrypt traffic, patch infrastructure, and tune detections. Document ownership, severity, compensating controls, and verification criteria. Re-test only within the approved scope.

## Alternatives

| Tool or method | Best for | Difficulty | License |
| --- | --- | --- | --- |
| Wireshark | A related workflow or comparison point | Verify upstream | Verify upstream |
| dumpcap | Complementary analysis | Verify upstream | Verify upstream |
| tshark | Validation or defense | Verify upstream | Verify upstream |
...
Use tcpdump when the objective is clearly defined, the data or target is authorized, the expected output is understood, and a reviewer can reproduce the observation.
...
See the related category index in [`tools/network-analysis/README.md`](../network-analysis/README.md) and compare with the alternatives listed above.
...
1. [tcpdump official repository](https://github.com/the-tcpdump-group/tcpdump)
2. [tcpdump official website](https://www.tcpdump.org/)
3. [tcpdump official documentation](https://www.tcpdump.org/manpages/tcpdump.1.html)
