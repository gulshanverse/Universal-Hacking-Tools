...
RELATIONSHIP_REVIEW_TYPES = {"related-to-vulnerability", "related-from-vulnerability", "related-to-technique", "technique-related-from", "affects-technology", "technology-affected-by", "mitigated-by", "control-for"}
...
def trust_report():
    ...
    for doc in docs:
        ...
        entity_trust.append({"entity": key, "name": doc["name"], "type": doc["type"], "trust_score": entity_score, "verification_status": verification.get("status", "unverified"), "confidence": verification.get("confidence", "unknown"), "verification_method": verification_method(doc), "source_count": len(source_records(doc)), "relationship_count": degree.get(key, 0), "claim_count": len(claim_by_entity.get(key, []))})
