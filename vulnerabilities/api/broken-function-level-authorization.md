---
id: broken-function-level-authorization
type: vulnerability
...
> Defensive knowledge page for an api fails to enforce permissions for sensitive functions or actions.

## Description

An API fails to enforce permissions for sensitive functions or actions. Assess the concrete implementation and trust boundary; the class name alone does not prove exploitability.
...
Potentially affected technologies include Apis, Authorization, Access Control. Verify the implementation and version before assigning a finding.
...
Apply Least Privilege, Secure Logging as appropriate. Fix the root cause, use secure defaults, minimize privileges, and verify the fix with a repeatable test.
...
[CWE-862](https://cwe.mitre.org/) — verify the precise mapping for the implementation under review.
...
* [Broken Function Level Authorization source](https://owasp.org/API-Security/editions/2023/en/0xa5-broken-function-level-authorization/)
