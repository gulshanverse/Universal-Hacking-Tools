...
> Defensive knowledge page for an unauthorized party uses a valid session token or state.

## Description

An unauthorized party uses a valid session token or state. Assess the concrete implementation and trust boundary; the class name alone does not prove exploitability.
...
Potentially affected technologies include Sessions, Cookies, Https. Verify the implementation and version before assigning a finding.
...
Apply Secure Cookies, Encryption, Secure Logging as appropriate. Fix the root cause, use secure defaults, minimize privileges, and verify the fix with a repeatable test.
...
* [Session Hijacking source](https://cwe.mitre.org/data/definitions/384.html)
