...
Phase 6 adds `lab-catalog.json` with the documentation-only, guided, and executable inventory; `lab-health.json` with safety and metadata findings; and `lab-report.json` with deterministic executable-lab coverage counts. These reports are generated from the classified Markdown lab pages, JSON-compatible YAML definitions, and committed synthetic fixtures.
